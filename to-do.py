...
class TodoApp:
    def __init__(self, root):
        ...
        self.root.geometry("400x600")  # Set the window size to 400x600 pixels

        # Load and set the background image
        try:
            image = Image.open("file.png")  # Use the uploaded image filename here
            image = image.resize((400, 600), Image.LANCZOS)  # Resize image to fit the window
        ...
        # Load existing tasks from the text file
        self.tasks = self.load_tasks()

        # Title label with a classic cream-white background
        self.title_label = tk.Label(root, text="To-Do List", font=("Georgia", 20, "bold"), bg="#8B4513", fg="#F5F5F5")
        ...
        # Add task title button with coffee and green colors
        ...
        self.task_listbox = tk.Listbox(root, width=35, height=5, font=("Georgia", 12), bg="#A5D6A7", fg="#6F4E37")
        self.canvas.create_window(200, 350, window=self.task_listbox)  # Position listbox on the canvas

        # Mark as done button with dark green background
        self.mark_button = tk.Button(root, text="Mark as Done", command=self.mark_task, bg="#2E7D32", fg="white", font=("Georgia", 12))
        self.canvas.create_window(200, 450, window=self.mark_button)  # Position button on the canvas

        # Edit task button
        self.edit_button = tk.Button(root, text="Edit Task", command=self.edit_task, bg="#6F4E37", fg="white", font=("Georgia", 12))
        self.canvas.create_window(200, 490, window=self.edit_button)  # Position button on the canvas

        # Delete task button with light coffee and red color
        self.delete_button = tk.Button(root, text="Delete Task", command=self.delete_task, bg="#C4A484", fg="red", font=("Georgia", 12))
    ...
    def load_tasks(self):
        ...
        if os.path.exists("tasks.txt"):
            with open("tasks.txt", "r") as file:
                for line in file:
                    title, description, category, completed = line.strip().split(',')
                    tasks.append({
                        "title": title,
    ...
    def save_tasks(self):
        with open("tasks.txt", "w") as file:
            for task in self.tasks:
                file.write(f"{task['title']},{task['description']},{task['category']},{task['completed']}\n")
